...
export default function MeetingResults({
  ...
  const hasAnyConsensus = meeting.groups.some(
    (group) => group.consensusReached,
  );
  const _allGroupsHaveRankings = meeting.groups.every(
    (group) => group.rankings.length > 0,
  );
  const someGroupsHaveRankings = meeting.groups.some(
    (group) => group.rankings.length > 0,
  );
  ...
  const expectedRewards = hasAnyConsensus
    ? calculateExpectedRewards()
    : new Map();
  const userRewards = userProfile
    ? expectedRewards.get(userProfile.principal.toString())
    : null;

  const totalRep = Array.from(expectedRewards.values()).reduce(
    (sum, r) => sum + r.rep,
    0,
  );
  const totalPhil = Array.from(expectedRewards.values()).reduce(
    (sum, r) => sum + r.phil,
    0,
  );
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                {hasAnyConsensus
                  ? t.consensus.consensusReachedDesc ||
                    "Consensus reached and tokens distributed"
                  : someGroupsHaveRankings
                    ? t.consensus.finalizationComplete ||
                      "Finalization complete - consensus not reached"
                    : t.consensus.awaitingRankings ||
                      "Finalization complete - awaiting rankings"}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-primary/10">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t.consensus.totalREPDistributed || "Total REP Distributed"}
                  </p>
              ...
              <div className="flex items-center gap-3 p-4 rounded-lg bg-accent/10">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t.consensus.totalPHILDistributed ||
                      "Total PHIL Distributed"}
                  </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-4 rounded-lg bg-muted/50">
                <p className="text-sm text-muted-foreground mb-1">
                  {t.consensus.repEarned || "REP Earned"}
                </p>
              ...
              <div className="text-center p-4 rounded-lg bg-muted/50">
                <p className="text-sm text-muted-foreground mb-1">
                  {t.consensus.philEarned || "PHIL Earned"}
                </p>
            ...
            {tokenBalance && (
              <div className="mt-4 pt-4 border-t">
                <p className="text-sm text-muted-foreground mb-2">
                  {t.consensus.currentBalance || "Current Balance"}
                </p>
                <div className="flex justify-around">
                  <div className="text-center">
                    <p className="text-lg font-semibold text-primary">
                      {Number(tokenBalance.rep)} REP
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-semibold text-accent">
                      {Number(tokenBalance.phil)} PHIL
                    </p>
                  </div>
                </div>
              </div>
      ...
      {!hasAnyConsensus && someGroupsHaveRankings && (
        ...
        >
          ...
          <AlertTitle className="text-yellow-900 dark:text-yellow-100">
            {t.consensus.consensusNotReached}
          </AlertTitle>
          <AlertDescription className="text-yellow-800 dark:text-yellow-200">
            {t.consensus.noConsensusMessage ||
              "The meeting has been finalized, but no group reached consensus. Rankings were submitted but did not meet the consensus threshold. No REP or PHIL tokens were distributed."}
          </AlertDescription>
        </Alert>
      )}
      ...
      {hasAnyConsensus && (
        ...
        >
          ...
          <AlertTitle className="text-green-900 dark:text-green-100">
            {t.consensus.consensusAchieved || "Consensus Achieved!"}
          </AlertTitle>
      ...
      {meeting.groups.map((group, groupIndex) => {
        ...
        return (
          <Card
            key={group.members.map((m) => m.toString()).join("-") || groupIndex}
          >
            <CardHeader>
              <div className="flex items-start justify-between">
                <div>
                  <CardTitle>
                    {t.consensus.group || "Group"} {groupIndex + 1}
                  </CardTitle>
                  <CardDescription>
                    {group.members.length} {t.consensus.members || "members"} •{" "}
                    {group.rankings.length}{" "}
                    {t.consensus.rankingsSubmitted || "rankings submitted"}
                    {allMembersRanked &&
                      ` • ${t.consensus.allParticipated || "All members participated"}`}
                  </CardDescription>
                </div>
                <Badge
                  variant={group.consensusReached ? "default" : "secondary"}
                >
              ...
              <div className="space-y-4">
                  ...
                  <Alert variant="default" className="bg-muted/50">
                    ...
                    <AlertDescription className="text-sm">
                      {group.members.length === 3 &&
                        (t.consensus.consensusRequirement3 ||
                          "All 3 members must submit identical rankings for consensus.")}
                      {group.members.length >= 4 &&
                        group.members.length <= 5 &&
                        (t.consensus.consensusRequirement45 ||
                          "At least 3 members must submit identical rankings for consensus.")}
                      {group.members.length === 6 &&
                        (t.consensus.consensusRequirement6 ||
                          "At least 4 members must submit identical rankings for consensus.")}
                      {group.members.length > 6 &&
                        (t.consensus.consensusRequirementAll ||
                          "All members must submit identical rankings for consensus.")}
                    </AlertDescription>
                  </Alert>
                )}
                    ...
                    {group.members.map((member) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {memberRewards &&
                          (memberRewards.rep > 0 || memberRewards.phil > 0) ? (
                            ...
                            <span className="text-sm text-muted-foreground">
                              {t.consensus.noRewards || "No rewards"}
                            </span>
                          )}
                        </div>
                      );
                ...
                {group.rankings.length > 0 && (
                  <>
                    ...
                    <div>
                      <h4 className="font-semibold mb-2">
                        {t.consensus.rankingsSubmitted}
                      </h4>
                      <p className="text-sm text-muted-foreground">
                        {group.rankings.length} {t.consensus.of || "of"}{" "}
                        {group.members.length}{" "}
                        {t.consensus.membersSubmittedRankings ||
                          "members submitted rankings"}
                      </p>
                      {allMembersRanked && !group.consensusReached && (
                        <p className="text-sm text-yellow-600 dark:text-yellow-400 mt-1">
                          {t.consensus.allRankedNoConsensus ||
                            "All members ranked, but rankings did not match sufficiently for consensus."}
                        </p>
                      )}
                    </div>
                  </>
